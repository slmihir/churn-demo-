...
export default function Analytics() {
  ...
  const riskDistribution = useMemo(() => {
    ...
    // Use ML predictions instead of static churnRisk field
    const predictions = mlPredictions.predictions;
    const lowRisk = predictions.filter((p: any) => (p.churnProbability || 0) < 0.5).length;
    const mediumRisk = predictions.filter((p: any) => (p.churnProbability || 0) >= 0.5 && (p.churnProbability || 0) < 0.8).length;
    const highRisk = predictions.filter((p: any) => (p.churnProbability || 0) >= 0.8).length;
